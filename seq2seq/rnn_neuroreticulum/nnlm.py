...
def main():
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('nnlm' + 'embedding'):
            ...
            embeddings = tf.nn.l2_normalize(embeddings, 1) #范化的 按行还是按列进行范化
        ...
        def infer_output(input_data):
            ...
            input_data_emb = tf.nn.embedding_lookup(embeddings,input_data) #选取一个张量里面索引对应的元素
            input_data_emb = tf.reshape(input_data_emb, [-1, args.win_size * args.word_dim]) #行数不变 宽度改变
            ...
            return output
        outputs = infer_output(input_data) #input_data 数据格式
        one_hot_targets = tf.one_hot(tf.squeeze(targets), args.vocab_size, 1.0, 0.0) #把这个映射成one_hot形式
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for e in range(args.num_epochs): #迭代的次数
