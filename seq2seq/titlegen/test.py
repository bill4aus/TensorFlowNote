...
with open('./toutiao_cat_data.txt','r',encoding='utf-8') as f:
    textlinesline = f.readlines()
    for line in textlinesline:
        try:
            linearray=line.split('_!_')
            title = linearray[3].strip().replace('\\n','').replace('\\t','')
            keyword_ = linearray[4].strip().replace('\\n','').replace('\\t','').split(',')
            # print('title:'+title)
...
en_data = [keywords for title,keywords in corpus]
